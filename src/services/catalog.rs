...
impl CatalogService {
    ...
    pub async fn create_catalog(&self, provider_id: &str, catalog: Catalog) -> Result<Catalog, ServiceError> {
    ...
    pub async fn select(&self, provider_id: &str, items: Vec<String>) -> Result<Vec<Item>, ServiceError> {
        // Business logic for item selection
        // For now, just get the catalog and filter the selected items
        let catalog = self.storage.get_catalog(provider_id).await?;
        
        // Find the selected items
        let selected_items: Vec<Item> = catalog.items
            .into_iter()
            .filter(|item| items.contains(&item.id))
            .collect();
            
        if selected_items.is_empty() {
            return Err(ServiceError::NotFound("No matching items found".to_string()));
        }
    ...
    pub async fn on_select(&self, _provider_id: &str, items: Vec<Item>) -> Result<Quotation, ServiceError> {
